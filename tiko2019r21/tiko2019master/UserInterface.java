...
public class UserInterface {
	...
	/* Käynnistysoperaatio, joka sisältää päävalikon ja asiakkaan hallinta-valikon*/ 
    public boolean run() {
        System.out.println("***********************");
        ...
        while (running) {
            if (asiakas != null) {
				...
				System.out.println("*******************************");
				System.out.println("*  5. Raporttimenu            *");
                System.out.println("*******************************");
                String cmd = inputReader.nextLine();
                switch (cmd) {
                    case "1":
    ...
    /* Operaatio, jossa lisätään asiakas tietokantaan*/
    ...
    /* Operaatio, jossa poistetaan asiakas tietokannasta ja samalla hänen projektit*/
	...
	/* Operaatio tarvikemenusta. Täällä luodaan uusia tarvikkeita tietokantaan*/
    private void hallitseTarvikkeita() {
        ...
        while (running) {
            ...
            System.out.println("*  2. Näytä tarvikelista         *");
    ...
    /* Operaatio, joka luo uuden tarvikkeen ja asettaa sille vaadittavat arvot*/
	...
	/* Operaatio, jolla haetaan käsiteltävä asiakas-
	 *Myös asiakkaiden listaamisen ominaisuus kuuluu tähän operaatioon
	 */
    ...
    /* Operaatio lisää projektin käsiteltävissä olevalle asiakkaalle tietokantaan
	 */
    ...
    /* Tulostaa asiakkaan projektit näytölle*/
    ...
    /* Operaatiossa valitaan asiakkaan projekti, johon lisätään työsuorituksia.*/
    ...
    /* Operaatio, joka hakee asiakkaan projektit, kun asiakas != null
	 * tärkeä apuoperaatio ohjelman sisällä
	 */
	...
	/* Projektimenu, jossa käsitellään valittua projektia*/
    private void projektiMenu() {
        ...
        while (running) {
            ...
            System.out.println("*-----------------------------------------*");
            System.out.println("*  3. Tyhjennä projektivalinta            *");
            System.out.println("*******************************************");
            String cmd = inputReader.nextLine();
            switch (cmd) {
                case "1":
	...
	/* Operaatio hakee työsuoritukset projektille.*/
    private void haeSuoritukset(boolean printataanko){
        ...
        try{
            ...
            while(rs.next()){
                ...
                if(printataanko){
                    System.out.println(rs.getInt(1) + "-" + rs.getFloat(3) +
					"-" + rs.getString(4) + "-" + rs.getFloat(6));
                } else{
	...
	/* Suoritusmenu, jossa ensin valitaan aktiivinen suoritus ja sitä aletaan sitten käsittelemään.*/
	private void suoritusMenu() {
		...
		System.out.println("Käsiteltävä työsuoritus on:\n" + aktiivinenSuoritus + '\n');
            ...
            System.out.println("*  5. Poista tarvike suorituksesta        *");
            System.out.println("*******************************************");
    ...
    /* Työsuorituksen lisäämisoperaatio. Lisää työsuorituksen projektiin, joko urakan tai tavallisen*/
	...
	/* Operaatio tulostaa luodut tarvikkeet listana*/
    ...
    /* Hakee työsuorituksessa olevat tarvikkeet suoritusmenussa*/
	...
	/* Tarvike- tyyppinen operaatio, joka lisää tarvikkeen suoritukseen
	 * Jos tarviketta ei ole aiemmin luotu, niin operaatio vie käyttäjän
	 * luoUusiTarvike()- operaatioon ja käyttäjä voi suoraan luoda uuden tarvikkeen.
	 */
	...
	/* Operaatio poistaa tarvikkeen suorituksesta.
	 * Mikäli halutaan poistaa tarvikkeita, että sinne jää jotain, niin
	 * taulua UPDATE:taan. Mikäli kaikki lähtee, niin taulu poisteaan kokonaan.
	 * 
	 */
	private void poistaTarvikeSuorituksesta(){
        ...
        int i = 0;
        tarvikkeet.forEach((key,value) -> System.out.println(key.getId() +
		" - " + key.getNimi() + ": " + value + key.getType() ));
	...
	/* Operaatio lisää tarvikkeen työsuorituksen tarvikeluetteloon
	 * Jos kyseistä tarviketta on jo, niin tarvikeluetteloa UPDATE:taan
	 * ja jos tarvike on työsuoritukselle uusi, niin tarvikelle luodaan oma rivi
	 */
	...
	/* Operaatio päivittää työsuorituksen hinnan. Se ottaa siihen mukaan tarvikkeiden hinnan */
    ...
    /* Operaatio laskee projektin kaikkien työsuoritusten hinnat tyoprojekti- tauluun*/
    ...
    /*Tulostaa kaikki työsuorituksessa käytetyt tarvikkeet listana*/
	...
	/* Raporttimenu, jossa ajetaan raportit 1 ja 2 läpi*/
	...
	/* Raportti 1 R1: Muodosta hinta-arvio kohteeseen x, joka sisältää suunnittelua 3 tuntia, 
	 * asennustyötä 12 tuntia, 3 metriä sähköjohtoa sekä yhden pistorasian.  
	 */
    ...
    private void tulostaRaporttiR2(){
		...
		System.out.println("*************************");
		//Projektin osoite
		System.out.println("*Kohteen osoite: ");
		System.out.println("*" + aktiivinenProjekti.getOsoite());
		System.out.println("*************************");
	
        float kokonaisHinta = 0; //kokonaisHinta
        ...
        System.out.println("Tarvikkeet:");
        haeTarvikkeet();
		//Tulostetaan tarvikkeet
        for (Map.Entry<Tarvike,Integer> pair : aktiivinenSuoritus.getTarvikkeet().entrySet()) {
            System.out.println(pair.getKey().getNimi() + " " + pair.getValue() + pair.getKey().getType() + 
			" " + pair.getKey().getMyyntihinta() * pair.getValue() + "€");
        }
		float kotvahkepoisuus = (kttvh*0.5f)-100f;
		float laskutuslisa = 5f;
		float kaikki = kokonaisHinta + laskutuslisa;
		//Kokonaissumma
        System.out.println("\nKokonaishinta: " + kokonaisHinta + "€");
		System.out.println("Laskutuslisä on " + laskutuslisa + "€");
		//Kotitalousvähennyskelpoisuus
		System.out.println("\nKotitalousvähennyskelpoisuus: " + kotvahkepoisuus + "€");
		System.out.println("");
		System.out.println("Maksettava summa: " + kaikki + "€");
		reset();
    }

	/* Operaatio nollaa oliot.
	*/
    private void reset() {
